...
<template>
  ...
  >
    ...
    <div
      v-if="activeSession && sessionComplete"
      ...
      <div class="max-w-2xl w-full p-6 bg-halo-gray/30 backdrop-blur-sm rounded-lg shadow-2xl">
        ...
        <div class="flex space-x-4">
          ...
          <button
            ...
            :class="isStartingNewGame ? 'opacity-70 cursor-not-allowed' : ''"
          >
             <svg
                v-if="isStartingNewGame"
    ...
    >
      <div class="text-center">
        ...
        ></div>
        <p class="text-lg text-blue-300">Loading Halo location...</p>
      </div>
    </div>
...
import MapCanvas from '~/components/MapCanvas.vue'; // Assuming MapCanvas is in components
...
const score = ref(0); // Only for Quick Play
const hasGuessed = ref(false);
const currentId = ref(null);
const isLoadingInitial = ref(true); // For the very first load
const isLoadingNextLevel = ref(false); // For subsequent loads
const screenshotLoaded = ref(false);
const mapLoaded = ref(false);
const pendingGuess = ref(null);
const showModInfo = ref(false);
const isMac = ref(false);
// Remove isStartingNewGame as we are just navigating now
// const isStartingNewGame = ref(false);

// Session-related state
const activeSession = ref(null); // Stores the session ID if in session mode
const sessionData = ref({ currentRound: 0, maxRounds: 0, totalScore: 0 });
const sessionRounds = ref([]); // Stores details of completed rounds in a session
const sessionComplete = ref(false);
const sessionSettings = ref(null); // Store settings for 'Play Again' (might not be needed anymore)

const imagesLoaded = computed(() => screenshotLoaded.value && mapLoaded.value);

// Reset image loaded flags when screenshot changes
watch(screenshot, () => {
...
async function loadSession(sessionId) {
  ...
  try {
    ...
    sessionSettings.value = data.settings; // Still store settings in case needed elsewhere
    sessionData.value = {
...
async function confirmGuess() {
  ...
  try {
    ...
    if (!activeSession.value) {
      ...
      sessionRounds.value.push({
          round_number: data.sessionData.currentRound,
          score: data.score,
...
function nextScreenshot() {
  if (activeSession.value && sessionData.value.currentRound >= sessionData.value.maxRounds) {
...
function playAgainWithSameSettings() {
  // Simply navigate to the composer page
  router.push('/compose');
}
